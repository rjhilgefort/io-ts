...
describe('brand', () => {
  ...
  describe('decode', () => {
    ...
    it('should fail validating an invalid value', () => {
      const T = t.Int
      assertFailure(T.decode('a'), ['Invalid value "a" supplied to : Int'])
      assertFailure(T.decode(1.2), ['Invalid value 1.2 supplied to : Int'])
    })

    it('should fail with the last deserialized value', () => {
      const T = IntFromString
      assertFailure(T.decode('a'), ['cannot parse to a number'])
      assertFailure(T.decode('1.2'), ['Invalid value 1.2 supplied to : IntFromString'])
    })
  })
